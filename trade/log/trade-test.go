...
func main() {
	...
	stellar.SetupParam(float64(1000), *isMainNet, "https://horizon.stellar.org/")
	...
	tradeRequest := horizonclient.TradeRequest{Cursor: "5381383568494609-0"}

	tradeHandler := func(trade horizon.Trade) {
		//log.Println(trade.ID)
		cursor = trade.ID

		log.Println("cursor:", trade.ID, trade.LedgerCloseTime)
	}
